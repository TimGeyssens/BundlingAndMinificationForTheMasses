...
namespace Optimus.Umbraco.Installer
{
    ...
    {
        //Set the web.config full path
        const string FullPath = "~/web.config";
        ...
        public bool Execute(string packageName, XmlNode xmlData)
        {
            ...
            //Get attribute values of xmlData
            string name, type;
            var position = GetAttributeDefault(xmlData, "position", null);
            if (!GetAttribute(xmlData, "name", out name)) return result;
            if (!GetAttribute(xmlData, "type", out type)) return result;
            var preCondition = GetAttributeDefault(xmlData, "preCondition", null);
            ...
            if (modified)
            {
                try
                {
                    ...
                    document.Save(HttpContext.Current.Server.MapPath(FullPath));
                ...
                {
                    //Log error message
                    var message = "Error at execute AddHttpModule package action" + e.Message;
        ...
        public bool Undo(string packageName, System.Xml.XmlNode xmlData)
        {
            ...
            if (modified)
            {
                ...
                {
                    //Log error message
                    var message = "Error at undo AddHttpModule package action: " + e.Message;
                    LogHelper.Error<AddHttpModule>(message, e);
                }
            }
            return result;
        ...
        /// <param name="value">returns the attribute value from xmlData</param>
        /// <returns>True, when attribute value available</returns>
        private static bool GetAttribute(XmlNode xmlData, string attribute, out string value)
        {
            //Set result default to false
            bool result = false;
        ...
        /// <param name="defaultValue">The default value</param>
        /// <returns>The attribute value or the default value</returns>
        private static string GetAttributeDefault(XmlNode xmlData, string attribute, string defaultValue)
        {
            //Set result default value
            string result = defaultValue;
        ...
        {
            return helper.parseStringToXmlNode(
                "<Action runat=\"install\" undo=\"true/false\" alias=\"Umbundle.AddHttpModule\" "
                    + "position=\"beginning/end\" "
                    + "name=\"BundleModule\" "
