...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		fileTree: function(options, file, folder) {
			...
			if (options.fileFilter      === undefined ) options.fileFilter      = '*.*';
			...
			$(this).each( function() {
				...
				function showTree(element, dir) {
					...
					$.post(options.script, { dir: dir, fileFilter: options.fileFilter }, function (data) {
				...
				function bindTree(element) {
				    //$(element).find('LI A').on('dblclick', function () {
